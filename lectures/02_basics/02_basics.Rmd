...
.font90[
`r emo::ji("check")` **Explain why forecasting is both valuable and difficult**, using real-world business and policy examples.

`r emo::ji("check")` **Recognize the fundamental limits of forecasting**, including uncertainty, randomness, and the difference between prediction and explanation.

`r emo::ji("check")` **Distinguish forecasting problems from other analytics tasks**, such as regression and classification, and explain why time ordering matters.

`r emo::ji("check")` **Identify what types of phenomena are forecastable and which are not**, based on the presence or absence of systematic structure.

`r emo::ji("check")` **Describe the core components of a time series**, including trend, seasonality, and noise.

`r emo::ji("check")` **Explain the concept of a data-generating process (DGP)** and why forecasting models aim to approximate processes rather than predict exact outcomes.

`r emo::ji("check")` **Interpret forecasts as distributions rather than single-point guesses**, and articulate why this perspective is essential for decision-making.
]
...
knitr::include_graphics("../../figures/python_tesla.png")
```
...
**Image Source:** Tesla's preferred qualifications for a [Sr. Machine Learning Engineer](https://www.linkedin.com/jobs/view/sr-machine-learning-engineer-autobidder-at-tesla-4277234017/) confirming the importance of Python, the transition to ML-DL in forecasting, and use of the `Nixtla` group of packages that we will utilize this semester. So to graduate **#BeyondReady graduates, you need to be introduced to the state-of-the-art**, which is readily available in Python but not in R. 
]
...
```{bash, eval=FALSE}
conda create --name isa444 python=3.13
```
...
- **Download** the *Historical Hourly Loads by Class 2025 data* from [here](https://www.duke-energyohiocbp.com/Documents/LoadandOtherData.aspx).  
...
duke = (
  ...
  [['datetime', 'TOTAL']]
  .query("TOTAL >= 3055456 and datetime >= '2025-02-01'") # use and for multiple conditions #<<
  )
...
- For example, we can use data prior to 2025 for training a forecasting model, and data from 2025 for testing the model to evaluate its performance.
...
# Sample DataFrame
data = {
    "date": pd.date_range(start="2020-01-01", end= '2025-08-30', freq="D"),
    # random walk
    "value": np.cumsum(np.random.randn(2069))
}
...
- Download and inspect the [01_hotel_occupancy.csv](https://miamioh.instructure.com/courses/250359/files/38340355?module_item_id=6543235)
...
- Complete [Assignment 02](https://miamioh.instructure.com/courses/250359/quizzes/785565) on Canvas to reinforce your understanding of today's class.

<div style="background: #fefce8; border-left: 4px solid #facc15; padding: 18px 22px; margin-bottom: 25px; border-radius: 0 8px 8px 0;"><strong>Before You Begin:</strong>
    <p style="margin: 10px 0 0 0; font-size: 14px; color: #451a03;">This assignment has a single part, where I ask you to examine historical data associated with Columbus's average temperature by month. Make sure you have reviewed the materials below before you begin the assignment.</p>
    <ul style="margin: 12px 0 0 0; padding-left: 20px; font-size: 14px; color: #451a03;">
        <li><strong style="color: #000;">Recommended review for Class 02:</strong>
            <ul style="margin: 8px 0 0 0; padding-left: 20px;">
                <li><strong style="color: #000;">Slide deck</strong></li>
                <li><strong style="color: #000;">Zoom recording</strong></li>
            </ul>
        </li>
        <li style="margin-top: 12px;"><strong style="color: #000;">NotebookLM GenAI Summaries</strong> <span style="font-size: 13px; color: #451a03;">(optional, but a different way to engage with the material)</span>:
            <ul style="margin: 8px 0 0 0; padding-left: 20px;">
                <li><a style="color: #2563eb; text-decoration: underline;" href="https://youtu.be/mRBZktBmeqY" target="_blank" rel="noopener">Python for Forecasting.mp4</a> <span style="font-size: 13px; color: #451a03;">(6-minute video highlight)</span></li>
            </ul>
        </li>
    </ul>
</div>