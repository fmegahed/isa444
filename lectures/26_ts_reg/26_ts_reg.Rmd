...
subtitle: '26: Time Series Regression'
...
- Use regression to account for seasonality in a time series.
...
t = time(log_jj) # renamed year var from last class
q = cycle(log_jj) |> factor() #<<

model3 = lm(log_jj ~ t + q) #<<

# checking out how the data is set up for lm
model.matrix(model3) #<<
```
]
]
...
.pull-right[
.font80[
```{r j11_out, ref.label = 'jj11', out.width = '90%', echo=FALSE, fig.dim= c(4.1, 4.1)}
...
.pull-right[
.font80[
```{r j12_out, ref.label = 'jj12', out.width = '90%', echo=FALSE, fig.dim= c(4.1, 4.1)}
...
.pull-right[
.font80[
```{r j13_out, ref.label = 'jj13', out.width = '90%', echo=FALSE, fig.dim= c(4.1, 4.1)}
...
.font80[
```{r jj14, eval=FALSE}
# forecasting 12 quarters ahead
forecast_3yrs = 
  forecast::forecast(
    model4, h = 12, level = 95 #<<
    )

# a quick plot of the forecast
forecast::autoplot(forecast_3yrs) + #<<
  forecast::autolayer(fitted(forecast_3yrs)) + #<<
  ggplot2::theme_bw() +
...
.pull-right[
.font80[
```{r j14_out, ref.label = 'jj14', out.width = '90%', echo=FALSE, fig.dim= c(4.1, 4.1)}
...
# Model Comparison: BIC & Forecast Accuracy

.pull-left[
.font80[
```{r jj15, eval=FALSE}
# compare their BIC (lower better)
cbind(
  BIC(model_tslm), BIC(model4) #<<
)

# comparing their forecast accuracy
acc_values = rbind(
  forecast::accuracy(model_tslm),
  forecast::accuracy(model4)
)

row.names(acc_values) = c('reg w/ trend', 'reg w/ trend + season')
...
.pull-right[
.font60[
```{r j15_out, ref.label = 'jj15', out.width = '90%', echo=FALSE, fig.dim= c(4.1, 4.1)}
...
# Model 4's Assumptions

.pull-left[
.font80[
```{r jj16, eval=FALSE}
# using resplot or check residuals
resplot(
...
forecast::checkresiduals(model4)

```

```{r jj16a, out.width = '70%', echo=FALSE, fig.dim= c(4.1, 4.1)}
# using resplot or check residuals
resplot(
  res = model4$residuals, 
  fit = model4$fitted.values,
...
.pull-right[
.font60[
```{r j16_out, out.width = '80%', echo=FALSE, fig.dim= c(4.1, 4.1)}
forecast::checkresiduals(model4)
```
]
]
...
.center[.font90[**Is the `auto.arima()` model better for this dataset?**]]

```{r jj17, include=FALSE, eval=FALSE}
model5 = forecast::auto.arima(log_jj)
...
forecast::checkresiduals(model5)
```
